...
const configPath: string = path.resolve(__dirname, '../private/configurations/vite.config.toml');
const config = toml.parse(fs.readFileSync(configPath, 'utf-8'));

// Check required fields
if (!config.server?.cert_folder || !config.server?.host || !config.server?.port) {
  throw new Error('Missing essential server configuration fields in vite.toml');
}

// Resolve paths
const resolvePath = (file: string): string => path.resolve(config.server.cert_folder, file);
...
const files = fs.readdirSync(config.server.cert_folder).filter(file => file !== '.gitkeep');
if (files.length !== 2) {
  throw new Error(`There should be exactly two files in the certificate folder, but found ${files.length}.`);
}
...
if (!certFile || !keyFile) {
  throw new Error('Could not find both certificate and private key in the folder.');
}
...
const viteConfig = {
  plugins: [sveltekit(), startupDetailPlugin()],
  css: {
  ...
  server: {
    ...
    https: useHttps ? { cert: fs.readFileSync(certPath), key: fs.readFileSync(keyPath) } : undefined,
    hmr: {
      protocol: wsProtocol,
    ...
    proxy: {
      '/api': {
        ...
        onProxyReq: (proxyReq: ClientRequest, req: IncomingMessage) => {
          logEvent('info', `Proxy request method: ${proxyReq.method}, path: ${proxyReq.path}`);
